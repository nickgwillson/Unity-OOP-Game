...
public class BallControl : MonoBehaviour
{
    ...
    void ballDirection(){    //start ball is where the ball will go when you start the game, it should be random left or right
        ...
        if (ballDirection < 1){     //if number is 1 ball goes to the right
            playerBody.AddForce(new Vector2(20, -15) );          //addforce is a unity function, which acts like a force exertoning on a rigibody object
        }
        else{
            playerBody.AddForce( new Vector2(-20, -15) );    //Vector 2 is a unity function representing 2d postions/vectors
        }
    }

    // Start is called at the begining of the game, right before we start Updating our frames
    void Start(){
        playerBody = GetComponent< Rigidbody2D >();     //grabbing the rigidbody2d unity function, setting it to variable playerBody
    ...
    void BallReset(){                           //we call this function when we restart the game or the ball hits the wall
        playerBody.velocity = Vector2.zero;                  //we set velocity and postion back to beginning 
    ...
    void OnCollisionEnter2D(Collision2D ballCollision){
        ...
        if (ballCollision.collider.CompareTag("Player")){    //IF THE BALL COLLIDES WITH A PLAYER
            ...
            ballVelocity.x = playerBody.velocity.x;   // set the ball velocity equal to the playerBody velocity that wew defined at the top 
                                                      // ballVelocity.y = (playerBody.velocity.y / 2) + (ballCollision.collider.attachedRigidbody.velocity.y / 3);    //set vertical ball velocity to 
            ballVelocity.y = playerBody.velocity.y;    //set vertical ball velocity to 

            playerBody.velocity = ballVelocity;  //set velocity to respective velocity set in beginning
        }
